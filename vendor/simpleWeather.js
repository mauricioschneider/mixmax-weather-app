...
/**
* Most of the code was taken from simpleWeather v3.1.0 - http://simpleweatherjs.com
* with the exception of some fixes to make it work with node
*/

var YQL = require('yql');

module.exports = function simpleWeather(opts, cb) {
  let options = {
  ...
  if (options.location !== '') {
    ...
    cb(new Error('Could not retrieve weather due to an invalid location.'));
    return false;
  }

  let query = new YQL(weatherUrl);
  query.exec(processData);
  ...
  function processData(err, data) {
    ...
    if (data && data.query && data.query.results && data.query.results.channel.description !== 'Yahoo! Weather Error') {
      ...
      cb(new Error('There was a problem retrieving the latest weather information.'));
    }
  }
}
